		...
		var TempoTap = function ( app, modal ) {
			...
			function _make_ui ( q ) {
				...
				el_drawer.innerHTML = '<div class="pk_row pk_pgeq_els">' + 
					...
					'<canvas width="1000" height="200" style="image-rendering:pixelated;width:500px;height:100px;display:block;background:#F0F1F0"></canvas>'+
		...
		var x = new PKAudioFXModal ({
			...
			body: '<div class="pk_rec" style="user-select:none">' +
				...
				'<div class="pk_row">' +
				  ...
				  '<div><label>Waveform</label><canvas width="1000" height="200" style="image-rendering:pixelated;width:500px;height:100px;display:block;background:#F0F1F0"></canvas></div>'+
				'</div>' +
				'<div class="pk_row">' +
...
//			}],
			...
			setup:function( q ) {
					...
					var volctx = volcanvas.getContext('2d', {alpha:true,antialias:false});

					var freqcanvas = q.el_body.getElementsByTagName('canvas')[1];
					var freqctx = freqcanvas.getContext('2d', {alpha:true,antialias:false});
					var tempCanvas = document.createElement('canvas');
					tempCanvas.width = 500 * 2;
					tempCanvas.height = 100 * 2;
					var tempCtx = tempCanvas.getContext ('2d', {alpha:true,antialias:false});
