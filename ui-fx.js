(function ( w, d, PKAE ) {
	...
	function PKUI_FX ( app ) {
		...
		app.listenFor ('RequestFXUI_EffeCt', function () {
			app.fireEvent ('RequestSelect', 1);
			...
			var getvalue = function ( q ) {
				...
				if (auto) {
					...
					var input = q.getElementsByTagName('input')[0];
					value = [{val: input.value / 1}];
				}
			...
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_flexed"> <div class="pk_row"><ul class="menu">'+ 
				'<li data-solve="silence"><span data-solve="silence">Insert silence</span></li>' +
				'<li data-solve="gain"><span data-solve="gain">Gain</span></li>'+
				'<li data-solve="speed"><span data-solve="speed">Speed</span></li>'+
				'</ul></div><div class="pk_rows silence"><div class="divider"></div><div class="pk_row radio-btn"><div><input type="radio" class="pk_check" id="ifeq" name="rdslnc" value="beginning">'+ 
				'<label for="ifeq">Insert silence at beginning</label></div><br/>' +
				'<div><input type="radio" class="pk_check"  id="vgdja" name="rdslnc" checked value="cursor">'+
				'<label for="vgdja">Insert silence at current cursor (<span class="pkcdpk"></span>)</label></div></div>'+
				'<div class="divider"></div><div class="pk_row range"><div><label>Silence in seconds</label>'+
				'<input type="range" min="0.0" max="30.0" class="pk_horiz" step="0.01" value="5.0" /></div>'+
				'<span class="pk_val">5s</span></div><div class="pk_row button-btn">'+ 
				'<div><button class="pk_cancel">CANCEL</button></div>'+
				'<div><button class="pk_ok">OK</button></div>'+
				'</div></div>'+
				'<div class="pk_rows gain"><div class="divider"></div>'+
				'<div class="pk_row range-gain" style="border:none"><div><label>Gain percentage</label>' + 
				'<input type="range" class="pk_horiz" min="0.0" max="2.5" step="0.01" value="1.0" /></div>'+
				'<span class="pk_val">100%</span></div>' +
				'<div class="pk_row button" style="border:none;padding:0">'+
				'<select class="pk_selector">'+
				...
				'</select>'+
				'<div class="pk_row button-btn">'+ 
				'<div><button data-where="gain" class="pk_cancel">CANCEL</button></div>'+
				'<div><button data-where="gain" class="pk_ok">apply gain</button></div>'+
				'</div>'+
				'</div></div><div class="pk_rows speed"><div class="divider"></div>'+
				'<div class="pk_row range-speed" style="border:none"><div><label>Playback Rate</label>' + 
				'<input type="range" class="pk_horiz" min="0.2" max="2.0" step="0.05" value="1.0" /></div>'+
				'<span class="pk_val">1.0</span></div>'+
				'<div class="pk_row button" style="border:none;padding:0">'+
				'<select data-select="speed" class="pk_selector">'+
				'<option value="">PRESETS</option>'+
				'<option value="0.25">-1/4</option>'+
				'<option value="0.5">-1/2</option>'+
				'<option value="0.85">Slightly slower</option>'+
				...
				'<div class="pk_row button-btn">'+ 
				'<div><button data-where="gain" class="pk_cancel">CANCEL</button></div>'+
				'<div><button data-where="gain" class="pk_ok">apply rate</button></div>'+
				'</div>'+
				'</div>'+
				'</div>'+
				...
				'</div></div>',
			  setup:function( q ) {
					var li = q.el_body.querySelectorAll('.menu > li');
					var title = q.el.querySelector('.pk_modal_title');
					var OKay = q.el_body.querySelectorAll('.pk_ok');
					var Cancel = q.el_body.querySelectorAll('.pk_cancel');
					var rows = q.el_body.querySelectorAll('.pk_rows');
					
					
					Cancel.forEach((canc) => {
						canc.onclick = function (item) {
						
							let q = d.getElementsByClassName('pk_modal_back')[0];
							q.remove();
						}
					})
					OKay.forEach((ok) => {
					ok.onclick = function (j) {
						let q = d.getElementsByClassName('pk_modal_back')[0];
						console.log(j)

						switch (j.target.dataset.where){
							case "gain":
								var value = getvalue ( q );
								if (value[0].val != 1.0)
									app.fireEvent ('RequestActionFX_GAIN', value);
						...
						var input = d.getElementsByClassName('pk_horiz')[0];
						var value = input.value.trim() / 1;

						var radios = d.getElementsByClassName('pk_check');
						var offset = 0;
					...
					var di = d.createElement('span')
						di.className = 'sub-title'
						di.innerText = 'Insert silence'
						title.appendChild(di)
						...
						element.onclick = function (e) {
							...
							switch (e.target.dataset.solve) {
								case "silence":
									...
									return
								case "gain":
									di.innerHTML = 'Apply Gain to selected range <button class="pk_preview">Preview <span data-on="true" class="text">ON</span></button>'
									title.appendChild(di)
									e.target.parentNode.classList.add("active")
									e.target.classList.add("active")
									rows[1].classList.add("active")
									var preview = q.el.querySelector('.pk_preview');
									
									preview.onclick = function (item) {
										let text = item.target.querySelector('.text');
										if(text.dataset.on === "true"){
											text.innerText = "OFF";
									...
									return
								case "speed":
									di.innerHTML = 'Change speed <button class="pk_preview">Preview <span data-on="true" class="text">ON</span></button>'
									title.appendChild(di)
									e.target.parentNode.classList.add("active")
									e.target.classList.add("active")
									rows[2].classList.add("active")
									var preview = q.el.querySelector('.pk_preview');
									var rangeSpeed = q.el_body.querySelector('.speed input');
									var spanSpeed = q.el_body.querySelector('.speed span');
	  
									rangeSpeed.oninput = function() {
										spanSpeed.innerHTML = rangeSpeed.value;
										app.fireEvent ('RequestActionFX_UPDATE_PREVIEW', rangeSpeed.value/1);
									};
									preview.onclick = function (item) {
										let text = item.target.querySelector('.text');
										if(text.dataset.on === "true"){
					...
					var ranges = q.el_body.querySelector('.gain input');
					var spana = q.el_body.querySelector('.gain span');
					var selecter = q.el_body.querySelectorAll('.pk_selector');
					ranges.oninput = function() {
					  spana.innerHTML = ((ranges.value * 100) >> 0) + '%';
					  app.fireEvent ('RequestActionFX_UPDATE_PREVIEW', [{val: ranges.value / 1}]);
					};
					selecter.forEach((selec) => {
						selec.onchange = function( e ) {
							if(e.target.dataset.select){
								var ranges = q.el_body.querySelector('.speed input');
								var value = e.target.value.trim() / 1;
								console.log(value)
								var int = ranges.value;
								var spanSpeed = q.el_body.querySelector('.speed span');

								if(int <= 2 && Math.sign(int) !== -1){
									spanSpeed.innerHTML = int;
									app.fireEvent ('RequestActionFX_PREVIEW_SPEED', [{val: int / 1}]);
									  ranges.value = int
								}
								return;
							}
							var ranges = q.el_body.querySelector('.gain input');
							var value = e.target.value.trim() / 1;
							var int = parseFloat(value)+parseFloat(ranges.value)-1;
							if(int <= 2.5 && Math.sign(int) !== -1){
								spana.innerHTML = (( int * 100) >> 0) + '%';
								app.fireEvent ('RequestActionFX_UPDATE_PREVIEW', [{val: int / 1}]);
								  ranges.value = int
							}
