(function ( w, d, PKAE ) {
		...
		return [
			{
				...
				children : [
					...
					{
						name: 'Load From URL11111111',
						action: function ( e ) {
							console.log('------------Load From URL11111111-----------------')
								new PKSimpleModal({
								  ...
								  buttons:[
									{
										...
										callback: function( q ) {
											...
											{
												// LOAD FROM URL....
												// app.engine.LoadURL ( new URLSearchParams(window.location.search).get('url') );
	...
	// make panner buttons
		...
		btn_panner_left.innerHTML = '<strong>Journalist</strong> ';
		btn_panner_right.innerHTML = '<strong>Guest</strong> ';
		...
		const urlRecord = getUrlRecord();

		if (!urlRecord) {
			let ttmp = d.createElement('div');
			ttmp.className = 'pk_tmpMsg';
			ttmp.innerHTML = '<strong>You don\'t have recorded audio!</strong><br>Drag n drop an Audio File in this window, or click ' +
	...
	function _makeUIToolbar (UI) {
		...
		pk_timingcnv.width = 170;
		pk_timingcnv.height = 42;
		...
		if (is_chrome)
		{
			...
			pk_timingctx.fillStyle = "#F0F1F4";
			pk_timingctx.fillRect(0, 0, 170, 42);
			...
			for (var ii = 0; ii < 11; ++ii)
			{
				...
				curr_ctx.font = "30px Helvetica, Arial, sans-serif";
				curr_ctx.fontSize = "600";
				curr_ctx.textAlign = "center";
				curr_ctx.fillStyle = "#F0F1F4";
				curr_ctx.fillRect(0, 0, 18, 26);
				curr_ctx.fillStyle = "#253D5A";
		...
		total_duration.className = 'pk_total_durx';
		divS.appendChild( total_duration );
		
		var hover_duration = d.createElement( 'span' );
		hover_duration.textContent = '00:00:000';
		hover_duration.className = 'pk_hover_durx';
		divS.appendChild( hover_duration );
		timing.appendChild( divS );
		...
		var effects_btn = d.createElement ('button');
		effects_btn.setAttribute('tabIndex', -1);
		effects_btn.className = 'pk_btn icon-magic';
		effects_btn.innerHTML = '<span>Effects</span>';
		actions.appendChild ( effects_btn );

		effects_btn.onclick = function() {
			
			UI.fireEvent( 'RequestFXUI_EffeCt');
			this.blur();
		};
		...
		btn_back_jump.innerHTML = '<span>Undo (Shift + Z)</span>';
		actions.appendChild ( btn_back_jump );
		...
		btn_front_jump.innerHTML = '<span>Redo (Shift + Z)</span>';
		actions.appendChild ( btn_front_jump );
		...
		////////////////////////
		var btn_fade_in = d.createElement ('button');
		btn_fade_in.setAttribute('tabIndex', -1);
		btn_fade_in.className = 'pk_btn pk_fade_in icon-fadein';
		btn_fade_in.innerHTML = '<span>Seek (fade in)</span>';
		actions.appendChild ( btn_fade_in );

		btn_fade_in.onclick = function() {
			UI.fireEvent ('RequestActionFX_FadeIn');
		}
		var btn_fade_out = d.createElement ('button');
		btn_fade_out.setAttribute('tabIndex', -1);
		btn_fade_out.className = 'pk_btn pk_fade_out icon-fadeout';
		btn_fade_out.innerHTML = '<span>Seek (fade out)</span>';
		actions.appendChild ( btn_fade_out );
		btn_fade_out.onclick = function() {
			UI.fireEvent ('RequestActionFX_FadeOut');
		}
		var silence_btn = d.createElement ('button');
		silence_btn.setAttribute('tabIndex', -1);
		silence_btn.className = 'pk_btn icon-silence';
		silence_btn.innerHTML = '<span>Quit</span>';
		...
		btn_clear_selection.innerHTML = '<span>Save</span>';
		...
		save_btn.className = 'pk_btn icon-save';
		save_btn.innerHTML = '<span>Save</span>';
		selection.appendChild ( save_btn );
		// selection.appendChild ( btn_clear_selection );
		selection.appendChild ( silence_btn );
